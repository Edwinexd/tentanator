...
- `backend/` (Rust + Axum) is the single source of truth. Owns all domain logic, persistence (Turso), sampling, ICL, LLM calls, exports. The HTTP API *is* the model.
- `tui/` (Python + Textual) is a thin client over the HTTP API.
- `web/` (TanStack Start + React) is a thin client over the same HTTP API.
...
## Client parity (TUI ⇄ Web) is non-negotiable
...
- **Contract parity** means both clients fully and correctly cover the backend contract. The backend is authoritative; no business logic lives in a client.
- **Capability parity** means any task a user can complete in the Web GUI can also be completed in the TUI. Covering a route in `api.py` is necessary but not sufficient: the capability must be reachable through the TUI's own UI, not just its API layer.
